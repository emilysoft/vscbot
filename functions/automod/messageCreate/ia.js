...
const module = async (message) => {
    try {
        ...
        if(reply) {
            ...
            if(message.client.iaUser.get(message.author.id)?.length > 0) {
                prompt = `tu dijiste: ${message.client.iaUser.get(message.author.id)}, ahora yo digo: ${ask}`
            } else {
          ...
          .then(res => res.json())
            .then(res => {
                    let mm=res.response
                        ...
                        .replace(/¿/gm, "")
                        .replace(/:smile:/gim, "<:a_heart:1116461880924446741>")
                message.client.iaUser.set(message.author.id, mm)
