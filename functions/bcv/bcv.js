...
module.exports = async (message, reply) => {
  if (reply) {
    message.channel.send("Cargando...").then(async (msg) => {
      ...
      msg
        .edit({ content: "", embeds: [obj] })
        .catch((err) => console.error(err));
    });
  } else {
    const obj = await getData();
    message.channel.send({ embeds: [obj] }).catch((err) => console.error(err));
  }
  async function getData() {
    try {
      ...
      const fecha = $("div.pull-right.dinpro.center span.date-display-single");
      ...
      //.setFooter({text:`🕒${fecha.text()}`,iconURL: botAvatar});
      return exampleEmbed;
    } catch (err) {
      console.error(err);
      return err;
