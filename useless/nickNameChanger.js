...
/**
 ...
 module.exports = async message =>  {
    await message.guild.members.cache.find(member => {
        // if(member.user.bot) return 
        if(member.nickname != null)  {
            if(member.nickname.slice(-2) == "🎃") {
                console.log("cambiando nickname a" + member.nickname)
                member.setNickname(`${member.nickname.slice(0,-2)}`).catch(e => console.error(e))
            } 
        }
