  ...
  client.guilds.cache.find((guild) => {
    guild.members.cache.find((member) => {
      ...
      if (member.nickname != null) {
        let sql = `INSERT INTO users (id, usernames, nicknames) VALUES("${member.id}", "${member.user.username}", "${member.nickname}")`;
        connection.query(sql, (error) => {
          if (error) return console.error(error);
        });
      } else {
        let sql = `INSERT INTO users (id, usernames) VALUES("${member.id}", "${member.user.username}")`;
        connection.query(sql, (error) => {
          if (error) return console.error(error);
        });
      }
