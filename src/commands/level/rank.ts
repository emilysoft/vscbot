...
async function rank(interaction: ChatInputCommandInteraction, client: Client) {
  ...
  if (!guild)
    return interaction.reply("Este comando solo funciona en un servidor.");
  ...
  if (!rankData) {
    return interaction.reply(
      `Lo siento, ${user.username} no tiene datos de nivel aún.`,
    );
  }
  ...
  if (!userDB || !userDB.id || !serverDB || !serverDB.id)
    throw new Error("error con el comando rank");
  // **Añadir el Rank al objeto RankData**
  rankData.rank = (await client.db.levels.getRank(userDB.id, serverDB.id)) as
    | number
    | undefined;

  // 3. Formatear y Mostrar el resultado
  const progressPercent =
    (rankData.xpProgress / (rankData.xpProgress + rankData.xpToNextLevel)) *
    100;

  const embed = new EmbedBuilder()
    .setTitle(`📊 Perfil de Rango de ${user.displayName || user.username}`)
    .setDescription(`**Nivel:** ${rankData.level}`)
    .addFields(
      {
        name: "Total XP",
        value: rankData.totalXp.toLocaleString(),
        inline: true,
      },
      {
        name: "Posición",
        value: rankData.rank ? `#${rankData.rank}` : "N/A",
        inline: true,
      },
      {
        name: "Progreso",
        value: `Faltan ${rankData.xpToNextLevel} XP para el Nivel ${rankData.level + 1}`,
        inline: false,
      },
    )
