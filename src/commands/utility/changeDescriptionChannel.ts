    ...
    if (interaction.channel instanceof TextChannel) {
        try {
            ...
            if (interaction instanceof ChatInputCommandInteraction) {
                interaction.reply({ content: "La descripción del canal ha sido actualizada.", ephemeral: true });
            } else if (interaction instanceof Message) {
                interaction.reply("La descripción del canal ha sido actualizada.");
            }
        } catch (error) {
            console.error("Error al cambiar la descripción del canal:", error);
            if (interaction instanceof ChatInputCommandInteraction) {
                interaction.reply({ content: "Ocurrió un error al intentar cambiar la descripción del canal. Asegúrate de tener los permisos necesarios.", ephemeral: true });
            } else if (interaction instanceof Message) {
                interaction.reply("Ocurrió un error al intentar cambiar la descripción del canal. Asegúrate de tener los permisos necesarios.");
            }
        }
    } else {
        if (interaction instanceof ChatInputCommandInteraction) {
