    ...
    if (interaction.channel instanceof TextChannel) {
        try {
            ...
            if (interaction instanceof ChatInputCommandInteraction) {
                await interaction.reply({ content: `El nombre del canal ha sido cambiado a **${newName}**`, ephemeral: true });
            } else if (interaction instanceof Message) {
                await interaction.reply(`El nombre del canal ha sido cambiado a **${newName}**`);
            }
        } catch (error) {
            console.error("Error al cambiar el nombre del canal:", error);
            if (interaction instanceof ChatInputCommandInteraction) {
                await interaction.reply({ content: "Ocurrió un error al intentar cambiar el nombre del canal. Asegúrate de tener los permisos necesarios.", ephemeral: true });
            } else if (interaction instanceof Message) {
                await interaction.reply("Ocurrió un error al intentar cambiar el nombre del canal. Asegúrate de tener los permisos necesarios.");
            }
        }
    } else {
        if (interaction instanceof ChatInputCommandInteraction) {
