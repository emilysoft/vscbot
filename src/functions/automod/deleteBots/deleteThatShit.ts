...
export default {
  ...
  execute: async function (message: Message, client: Client) {
    ...
    try {
      ...
      // #Comandos
      if (channel.id == "1112164583344443433" && !message.channel.isThread()) {
        await handleGeneralChannel(message);
      }
...
async function handleGeneralChannel(message: Message): Promise<void> {
  ...
  if (isCommand && !author.bot) {
    message.delete();
    return;
  }

  // Borrar mensajes del bot "fmbot" después de 2 minuto
  if (
    author.id === FMBOT_USER_ID ||
    author.id === CHUU_USER_ID ||
    author.id === MUDAE_USER_ID
  ) {
 ...
 */
async function deleteTempMessage(
  message: Message,
  delay: number,
): Promise<void> {
  setTimeout(async () => {
