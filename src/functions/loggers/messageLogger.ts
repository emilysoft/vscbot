...
const module = async (message: Message, type: string, client: Client) => {
    try {
        ...
        if (type == "create") typeLog = ":green_circle:";
            ...
            throw new Error("Error al especificar el tipo de evento de mensaje para el messageLogger");
        }
        ...
        const channel_db = await client.db.channels.get(channel, serverDB.id) as DB_Channel;
