...
const recentVerifications = new Map<
  string,
  { timestamp: number; userTag: string }
>();
 ...
 */
async function handleWelcomeMessages(
  ...
  if (wasUnverified && isVerified) {
    const now = Date.now();
    recentVerifications.set(newMember.id, {
      timestamp: now,
    ...
    if (!welcomeChannel)
      return console.error("¡Canal de bienvenida no encontrado!");

    // --- Lógica del Mensaje de Bienvenida ---
    if (recentVerifications.size >= GROUP_THRESHOLD) {
      // Saludo grupal
      const userMentions = Array.from(recentVerifications.keys())
        .map((id) => `<@${id}>`)
        .join(" ");
      ...
      setTimeout(() => {
        webhookClient.deleteMessage(sentMessage.id).catch((err) => {
          console.error(
            "Error al intentar borrar el mensaje del webhook:",
            err,
          );
        });
    ...
    const pingChannel = (await guild.channels.fetch(
      PING_CHANNEL_ID,
    )) as TextChannel;
    if (pingChannel && pingChannel.type === ChannelType.GuildText) {
      const userMentionsForPing = Array.from(recentVerifications.keys())
        .map((id) => `<@${id}>`)
        .join(" ");
